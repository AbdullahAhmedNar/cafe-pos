...
exports.setupSimpleIPC = setupSimpleIPC;
...
function setupSimpleIPC() {
    try {
        // معالج بسيط للاختبار
        electron_1.ipcMain.handle('test:ping', async () => {
            return { success: true, message: 'pong' };
        });
        // معالج فتح الملفات
        electron_1.ipcMain.handle('openFile', async (event, filePath) => {
            try {
        ...
        electron_1.ipcMain.handle('printReport', async (event, filePath) => {
            try {
                if (fs.existsSync(filePath)) {
                ...
                electron_log_1.default.error('Error printing file:', error);
                return { success: false, error: 'حدث خطأ في طباعة الملف' };
            }
        });
        // معالج اختبار قاعدة البيانات
        electron_1.ipcMain.handle('test:db', async () => {
            try {
                ...
                electron_log_1.default.error('Database test error:', error);
                return { success: false, error: error.message };
            }
        });
        // معالج تسجيل الدخول مع قاعدة البيانات
        electron_1.ipcMain.handle('users:login', async (event, credentials) => {
            try {
                ...
                electron_log_1.default.info(`User ${username} logged in successfully`);
                return {
                    success: true,
                    data: {
                        user: {
                ...
                electron_log_1.default.error('Login error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('products:list', async (event, filters = {}) => {
            try {
                const db = (0, db_1.getDatabase)();
                ...
                electron_log_1.default.error('Products list error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('settings:get', async () => {
            try {
                ...
                electron_log_1.default.error('Settings get error:', error);
                return {
                    success: false,
        ...
        // معالج ملخص اليوم
        electron_1.ipcMain.handle('reports:dailySummary', async (event, date) => {
            try {
                const db = (0, db_1.getDatabase)();
                const targetDate = date || new Date().toISOString().split('T')[0];
                electron_log_1.default.info('Fetching daily summary for date:', targetDate);
        ...
        `;
                const summary = await new Promise((resolve, reject) => {
                    db.get(salesQuery, [targetDate], (err, row) => {
                        if (err) {
                            electron_log_1.default.error('Sales query error:', err);
                            reject(err);
                        }
                        else {
                            electron_log_1.default.info('Sales summary result:', row);
                            resolve(row || { total_sales: 0, orders_count: 0, avg_order_value: 0, total_discount: 0, total_tax: 0 });
                        }
                    });
                });
                // جلب إجمالي المنتجات المباعة
                const itemsSold = await new Promise((resolve, reject) => {
                    db.get(itemsSoldQuery, [targetDate], (err, row) => {
                        if (err) {
                            electron_log_1.default.error('Items sold query error:', err);
                            reject(err);
                        }
                        else {
                            electron_log_1.default.info('Items sold result:', row);
        ...
        `;
                const customers = await new Promise((resolve, reject) => {
                    db.get(customersQuery, [targetDate], (err, row) => {
                        if (err)
                            ...
                            resolve(row || { unique_customers: 0, total_orders: 0 });
                    });
                });
                return {
                    success: true,
                ...
                electron_log_1.default.error('Daily summary error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('reports:topProducts', async (event, limit = 10) => {
            try {
                const db = (0, db_1.getDatabase)();
                const query = `
          SELECT 
        ...
        `;
                ...
                electron_log_1.default.error('Top products error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('reports:sales', async (event, query) => {
            try {
                const db = (0, db_1.getDatabase)();
                const { from, to, groupBy = 'day' } = query;
                electron_log_1.default.info('Sales report query:', { from, to, groupBy });
        ...
        `;
                const sales = await new Promise((resolve, reject) => {
                    db.all(salesQuery, [from, to], (err, rows) => {
                        if (err) {
                            ...
                            electron_log_1.default.info('Sales report result:', rows);
                            resolve(rows || []);
                        }
                    });
                });
                ...
                electron_log_1.default.error('Sales report error:', error);
                return {
                    success: false,
        ...
        // معالج تصدير PDF
        electron_1.ipcMain.handle('reports:exportToPDF', async (event, data, type) => {
            try {
                const fs = require('fs');
                const path = require('path');
                // إنشاء مجلد التقارير إذا لم يكن موجوداً
                const reportsDir = path.join(electron_1.app.getPath('documents'), 'Tesla Cafe POS', 'Reports');
          ...
          WHERE DATE(date) BETWEEN ? AND ?
        `;
                const salesOverview = await new Promise((resolve, reject) => {
    ...
    <div class="report-container">
        ...
        <div class="footer">
            ...
            <div class="thank-you">شكراً لاستخدام نظامنا ☕</div>
                ...
                electron_log_1.default.info('HTML report generated successfully:', filePath);
                return {
                    success: true,
                    data: {
                        filename,
                ...
                electron_log_1.default.error('Report export error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('reports:overview', async (event, dateRange) => {
            try {
                const db = (0, db_1.getDatabase)();
                const { from, to } = dateRange || {
                    from: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        ...
        `;
                const salesOverview = await new Promise((resolve, reject) => {
                    db.get(salesOverviewQuery, [from, to], (err, row) => {
                        if (err) {
                            electron_log_1.default.error('Sales overview query error:', err);
                            reject(err);
                        }
                        else {
                            electron_log_1.default.info('Sales overview result:', row);
                            resolve(row || { total_orders: 0, total_sales: 0, avg_order_value: 0, total_discount: 0, total_tax: 0, unique_customers: 0 });
                        }
                    });
                });
        ...
        `;
                const productsOverview = await new Promise((resolve, reject) => {
                    db.get(productsOverviewQuery, [from, to], (err, row) => {
                        if (err)
                            reject(err);
                        else
                            resolve(row || { total_products: 0, total_categories: 0, total_items_sold: 0, total_revenue: 0 });
                    });
                });
                // إحصائيات طرق الدفع
        ...
        `;
                const paymentOverview = await new Promise((resolve, reject) => {
        ...
        `;
                const categoriesOverview = await new Promise((resolve, reject) => {
                ...
                electron_log_1.default.error('Overview report error:', error);
                return {
                    success: false,
        ...
        // معالج تصدير CSV
        electron_1.ipcMain.handle('reports:exportToCSV', async (event, data, filename) => {
            try {
                ...
                electron_log_1.default.error('CSV export error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('users:list', async () => {
            try {
                ...
                electron_log_1.default.error('Users list error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('users:create', async (event, userData) => {
            try {
                ...
                electron_log_1.default.error('Create user error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('users:update', async (event, userId, userData) => {
            try {
                ...
                electron_log_1.default.info(`User ${userId} updated successfully`);
                return {
                    success: true,
                ...
                electron_log_1.default.error('User update error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('users:delete', async (event, userId) => {
            try {
                const db = (0, db_1.getDatabase)();
                ...
                electron_log_1.default.info(`User ${userId} deleted successfully`);
                return {
                    success: true,
                    data: { id: userId }
                };
                ...
                electron_log_1.default.error('User deletion error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('products:create', async (event, product) => {
            try {
                ...
                const db = (0, db_1.getDatabase)();
                const result = await new Promise((resolve, reject) => {
                    db.run('INSERT INTO products (name, price, category, sku, stock, image) VALUES (?, ?, ?, ?, ?, ?)', [name, price, category, sku, stock, image], function (err) {
                        if (err)
                            reject(err);
                        else
                            resolve({ id: this.lastID });
                ...
                electron_log_1.default.error('Create product error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('products:update', async (event, id, product) => {
            try {
                const db = (0, db_1.getDatabase)();
                // إذا كان التحديث يحتوي على is_active فقط (تفعيل/إلغاء تفعيل)
                if (Object.keys(product).length === 1 && 'is_active' in product) {
                    await new Promise((resolve, reject) => {
                        db.run('UPDATE products SET is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [product.is_active, id], (err) => {
                            if (err)
                                reject(err);
                            else
                                resolve();
                    ...
                    await new Promise((resolve, reject) => {
                        db.run('UPDATE products SET name = ?, price = ?, category = ?, sku = ?, stock = ?, image = ?, is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [name, price, category, sku, stock, image, is_active, id], (err) => {
                            if (err)
                                reject(err);
                            else
                                resolve();
                ...
                electron_log_1.default.error('Update product error:', error);
                return {
                    success: false,
        ...
        // معالج حذف المنتجات
        electron_1.ipcMain.handle('products:delete', async (event, id) => {
            try {
                const db = (0, db_1.getDatabase)();
                await new Promise((resolve, reject) => {
                ...
                electron_log_1.default.error('Delete product error:', error);
                return {
                    success: false,
        ...
        // معالج تحديث المخزون
        electron_1.ipcMain.handle('products:updateStock', async (event, id, quantity) => {
            try {
                const db = (0, db_1.getDatabase)();
                await new Promise((resolve, reject) => {
                    db.run('UPDATE products SET stock = stock + ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [quantity, id], (err) => {
                        if (err)
                            reject(err);
                        else
                            resolve();
                ...
                electron_log_1.default.error('Update stock error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('products:getByBarcode', async (event, sku) => {
            try {
                const db = (0, db_1.getDatabase)();
                ...
                electron_log_1.default.error('Get product by barcode error:', error);
                return {
                    success: false,
        ...
        // معالج إنشاء الطلبات
        electron_1.ipcMain.handle('orders:create', async (event, order) => {
            try {
                electron_log_1.default.info('Received order data:', JSON.stringify(order, null, 2));
                ...
                const db = (0, db_1.getDatabase)();
                electron_log_1.default.info('Database connection established');
                // التأكد من أن total موجود وصحيح
                if (total === null || total === undefined || isNaN(total) || total < 0) {
                    electron_log_1.default.error('Invalid total received:', { total, items });
                    return {
                        success: false,
                ...
                if (isNaN(numericTotal) || isNaN(numericDiscount) || isNaN(numericTax) || isNaN(numericPaid)) {
                    electron_log_1.default.error('Non-numeric values detected:', { total, discount, tax, paid });
                    return {
                        success: false,
                        error: 'قيم غير صحيحة في الطلب'
                    };
                }
                electron_log_1.default.info('Order data validated:', {
                    total: numericTotal,
                ...
                const lastOrder = await new Promise((resolve, reject) => {
                ...
                electron_log_1.default.info('Creating order with params:', {
                    orderNumber,
                    total: numericTotal,
                    discount: numericDiscount,
                    tax: numericTax,
                ...
                const orderResult = await new Promise((resolve, reject) => {
                    db.run('INSERT INTO orders (order_no, date, total, discount, tax, paid, payment_method, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [orderNumber, dateString, numericTotal, numericDiscount, numericTax, numericPaid, payment_method, finalUserId], function (err) {
                        if (err) {
                            electron_log_1.default.error('Database insert error:', err);
                            reject(err);
                        }
                        else {
                            electron_log_1.default.info('Order created successfully with ID:', this.lastID);
                            resolve({ id: this.lastID });
                        }
                    });
                });
                ...
                for (const item of items) {
                    ...
                    const productInfo = await new Promise((resolve, reject) => {
                        db.get('SELECT name, stock FROM products WHERE id = ?', [item.product_id], (err, row) => {
                            if (err)
                                reject(err);
                            else
                                resolve(row);
                ...
                for (const item of items) {
                    ...
                    await new Promise((resolve, reject) => {
                        db.run('INSERT INTO order_items (order_id, product_id, qty, unit_price, subtotal) VALUES (?, ?, ?, ?, ?)', [orderResult.id, item.product_id, item.qty, item.unit_price, subtotal], (err) => {
                            if (err)
                                reject(err);
                            else
                                resolve();
                    ...
                    // تحديث المخزون
                    await new Promise((resolve, reject) => {
                        db.run('UPDATE products SET stock = stock - ? WHERE id = ?', [item.qty, item.product_id], (err) => {
                            if (err)
                                reject(err);
                            else
                                resolve();
                ...
                electron_log_1.default.error('Create order error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('orders:print', async (event, orderId) => {
            try {
                electron_log_1.default.info('orders:print handler called with orderId:', orderId);
                const db = (0, db_1.getDatabase)();
                // جلب بيانات الطلب
                const orderQuery = `
          SELECT 
        ...
        `;
                ...
                const settings = await new Promise((resolve, reject) => {
                    db.all(settingsQuery, ['cafe_name', 'currency_symbol', 'receipt_header', 'receipt_footer', 'cafe_address'], (err, rows) => {
                        if (err)
                            reject(err);
                        else
                            resolve(rows || []);
...
╚══════════════════════════════════════════════════════════════╝
        `;
                ...
                electron_log_1.default.error('Print receipt error:', error);
                return {
                    success: false,
        ...
        // معالج قائمة الطلبات
        electron_1.ipcMain.handle('orders:list', async (event, filters = {}) => {
            try {
                const db = (0, db_1.getDatabase)();
                let query = `
          SELECT 
        ...
        `;
                ...
                electron_log_1.default.error('Orders list error:', error);
                return {
                    success: false,
        ...
        // معالج تفاصيل الطلب
        electron_1.ipcMain.handle('orders:getById', async (event, id) => {
            try {
                const db = (0, db_1.getDatabase)();
                // جلب الطلب
                const order = await new Promise((resolve, reject) => {
                    db.get('SELECT o.*, u.username as user_name FROM orders o LEFT JOIN users u ON o.user_id = u.id WHERE o.id = ?', [id], (err, row) => {
                        if (err)
                            reject(err);
                        else
                            resolve(row);
                ...
                // جلب عناصر الطلب
                const items = await new Promise((resolve, reject) => {
                    db.all('SELECT oi.*, p.name as product_name FROM order_items oi LEFT JOIN products p ON oi.product_id = p.id WHERE oi.order_id = ?', [id], (err, rows) => {
                        if (err)
                            reject(err);
                        else
                            resolve(rows || []);
                ...
                electron_log_1.default.error('Get order error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('orders:generateOrderNumber', async () => {
            try {
                const db = (0, db_1.getDatabase)();
                // جلب آخر رقم طلب من قاعدة البيانات
                const lastOrder = await new Promise((resolve, reject) => {
                ...
                electron_log_1.default.error('Generate order number error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('orders:resetCounter', async () => {
            try {
                const db = (0, db_1.getDatabase)();
                ...
                electron_log_1.default.info('Order counter reset successfully');
                return {
                    success: true,
                ...
                electron_log_1.default.error('Reset counter error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('orders:cleanOldOrders', async () => {
            try {
                const db = (0, db_1.getDatabase)();
                ...
                electron_log_1.default.info('Old orders cleaned successfully');
                return {
                    success: true,
                ...
                electron_log_1.default.error('Clean old orders error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('products:generateSKU', async () => {
            try {
                ...
                electron_log_1.default.error('Generate SKU error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('settings:set', async (event, settings) => {
            try {
                ...
                electron_log_1.default.error('Settings set error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('settings:reset', async () => {
            try {
                ...
                electron_log_1.default.error('Settings reset error:', error);
                return {
                    success: false,
        ...
        // معالج اختيار الصورة
        electron_1.ipcMain.handle('fs:selectImage', async (event) => {
            try {
                ...
                electron_log_1.default.error('Select image error:', error);
                return {
                    success: false,
        ...
        electron_1.ipcMain.handle('print:createWindow', async (event, htmlContent) => {
            try {
                ...
                electron_log_1.default.error('Create print window error:', error);
                return {
                    success: false,
        ...
        electron_log_1.default.info('Simple IPC handlers setup successfully');
    }
    catch (error) {
        electron_log_1.default.error('Failed to setup simple IPC handlers:', error);
        throw error;
    }
}
